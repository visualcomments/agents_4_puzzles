#!/usr/bin/env python3
"""
Standalone launcher for agents_4_puzzles 444-cube prompt sweep.

This script was generated from the Colab notebook:
agents_4_puzzles_cube444_full_prompt_sweep_kaggle_feedback_FIXED_single_inline_kaggle_json.ipynb
...
5. writes the 444-cube prompt-sweep runner into colab/;
...
def default_workspace_dir() -> str:
    ...
    env_value = os.environ.get("CUBE444_WORKDIR")
    if env_value:
        return str(Path(env_value).expanduser())
    return str(get_script_dir() / "cube444_prompt_sweep_workspace")


def default_archive_path() -> str:
    """Default zip path next to this script; used only for local_zip_path/upload_zip modes."""
    env_value = os.environ.get("CUBE444_ARCHIVE_PATH")
    if env_value:
        return str(Path(env_value).expanduser())
    return str(get_script_dir() / "agents_4_puzzles_444_prompt_sweep_pipeline_checked.zip")
...
def ensure_writable_dir(path: Path, label: str) -> Path:
    ...
    try:
        ...
        fallback = (Path.cwd() / "cube444_prompt_sweep_workspace").resolve()
...
def write_runner(repo_dir: Path, *, overwrite: bool = True) -> Path:
    runner_path = repo_dir / "colab" / "cube444_prompt_sweep_kaggle_feedback.py"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Clone/unpack agents_4_puzzles, install dependencies, configure credentials, and run 444-cube prompt sweep."
    )
    ...
    workspace.add_argument("--archive-path", default=default_archive_path(), help="Path to repository zip for local_zip_path/drive_zip/upload_zip modes. Defaults to a zip next to this script or CUBE444_ARCHIVE_PATH.")
    ...
    workspace.add_argument("--workdir", default=default_workspace_dir(), help="Writable workspace directory. Defaults to ./cube444_prompt_sweep_workspace next to this script or CUBE444_WORKDIR.")
    ...
    main.add_argument("--competition", default="cayley-py-444-cube")
    main.add_argument("--kaggle-competition", default="cayley-py-444-cube")
    ...
    main.add_argument("--run-name", default="cube444_full_prompt_sweep")
    ...
    main.add_argument("--output-root", default="runs/cube444_prompt_sweep")
    ...
    budget.add_argument("--baseline", default="competitions/cayley-py-444-cube/baselines/cube444_baseline_solver.py")
    ...
    scoring.add_argument("--message-prefix", default="444-cube prompt sweep")
