...
def test_report_is_recoverable_excludes_credentials_required():
    report = 'g4f:r1-1776: fixer iteration 1 credentials required (Add a "api_key")'
    assert rpp._report_requires_credentials(report) is True
    assert rpp._report_is_recoverable(report) is False



def test_attempt_recovery_rounds_skips_credentials_reports(monkeypatch, tmp_path):
    monkeypatch.setenv('AGENTLAB_G4F_RECOVERY_ROUNDS', '3')
    called = []
    ...
    ok, report = rpp.attempt_recovery_rounds(
        ...
        max_iters=4,
        baseline_code='def solve(vec):\n    return [], list(vec)\n',
        generation_reports=['g4f:gpt-4: fixer iteration 1 credentials required (Add a "api_key")'],
    )
    assert ok is False
    assert report is None
    assert called == []
...
def test_run_hybrid_codegen_search_stops_refinement_on_credentials_only_frontier(monkeypatch, tmp_path):
    calls = {'planner': 0}

    def fake_generate(*args, **kwargs):
        calls['planner'] += 1
        return [rpp.PlanCandidate(plan_text='plan', planner_model='local:planner', score=1.0, variant_index=1)]

    monkeypatch.setattr(rpp, 'generate_plan_candidates', fake_generate)
    monkeypatch.setattr(
        rpp,
        'try_generate_with_model',
        lambda **kwargs: (False, 'g4f:r1-1776: fixer iteration 1 credentials required (Add a "api_key")'),
    )

    ok, reports, archive, plan, planner_model, winner_model = rpp.run_hybrid_codegen_search(
        planner_models=['local:planner'],
    ...
    assert plan == 'plan'
    assert planner_model == 'local:planner'
    assert reports
