#!/usr/bin/env python3
"""Validator for CayleyPy Transposons solver output.
...
- moves is either:
    * list[str] of move tokens like T[i,j,k]
    * a '.'-separated string of move tokens
    * the string 'UNSOLVED' (accepted for template/baseline)
- applying the moves (as permutations) to the input must reach the
  corresponding central_state loaded from data/graphs_info.json (by state size)
...
def _load_graphs_info() -> Dict[str, Any]:
    here = Path(__file__).resolve().parent
    p = here / "data" / "graphs_info.json"
    if not p.exists():
        raise FileNotFoundError(f"Missing graphs_info.json at {p}")
...
def main() -> None:
    ...
    graphs = _load_graphs_info()

    n = len(vec)
    info = graphs.get(str(n))
    if not info:
        print(f"[!] graphs_info.json does not contain state_size={n}", file=sys.stderr)
        raise SystemExit(1)

    central_state = info["central_state"]
    generators: Dict[str, List[int]] = info["generators"]
