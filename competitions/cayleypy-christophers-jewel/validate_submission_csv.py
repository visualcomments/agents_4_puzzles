#!/usr/bin/env python3
"""Validate a cayleypy-christophers-jewel Kaggle submission by replaying every path.

Checks the actual competition contract:
- `test.csv` columns: initial_state_id, initial_state
...
def main() -> None:
    ...
    if len(central_state) != 48:
        raise SystemExit(f"[!] central_state length must be 48 for Christopher's Jewel, got {len(central_state)}")
    if len(generators) != 12:
        raise SystemExit(f"[!] expected 12 generators, got {len(generators)}")
