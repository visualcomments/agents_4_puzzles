#!/usr/bin/env python3
"""Strict validator for CayleyPy Christopher's Jewel solver output.

Solver contract:
    python solve_module.py "[...]" -> {"moves": ..., "sorted_array": ...}

For cayleypy-christophers-jewel a successful generated solver must emit a real
legal path that transforms the provided initial_state to puzzle_info.json
central_state. UNSOLVED is rejected by default because it is not a valid
generated solution for prompt-sweep success.
"""
...
def _parse_moves(moves: Any, *, allow_unsolved: bool) -> List[str]:
    if isinstance(moves, str):
        ...
        if s.upper() == 'UNSOLVED':
            if allow_unsolved:
                return []
            raise ValueError("UNSOLVED is rejected for generated Christopher's Jewel solvers")
        if not s:
            return []
        return s.split('.')
...
def main() -> None:
    ...
    allow_unsolved = bool(args.allow_unsolved or os.getenv('JEWEL_ALLOW_UNSOLVED', '').lower() in {'1', 'true', 'yes', 'on'})
    ...
    if len(vec) != len(central_state):
        ...
        raise SystemExit(1)
    if len(central_state) != 48:
        print(f'[!] central_state length must be 48 for Christopher\'s Jewel, got {len(central_state)}', file=sys.stderr)
        raise SystemExit(1)
    if len(generators) != 12:
        print(f'[!] expected 12 generators, got {len(generators)}', file=sys.stderr)
        raise SystemExit(1)
