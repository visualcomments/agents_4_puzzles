...
# CopilotSession is only usable with zendriver; keep import safe so g4f can be imported without optional deps.
if has_nodriver:
    from .Copilot import click_trunstile
else:
    async def click_trunstile(*_args, **_kwargs):
        raise MissingAuthError("zendriver is required for CopilotSession")
