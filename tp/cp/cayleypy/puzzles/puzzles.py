...
class Puzzles:
    ...
    @staticmethod
    def picture_cube333() -> CayleyGraphDef:
        """Cayley graph for the Picture Cube puzzle, a variant of the 333
        Rubik’s Cube where each face has an oriented image. The graph is for the IHES PictureCube puzzle:
        the subgroup of S_72 generated by the allowed moves,
        acting on 72 picture-oriented facelets.
        Benchmark challenge: https://www.kaggle.com/competitions/cayleypy-ihes-cube
        """
        generator_names = list(PICTURE_CUBE_333_ALLOWED_MOVES.keys())
        generators = [PICTURE_CUBE_333_ALLOWED_MOVES[k] for k in generator_names]
        central_state = list(range(len(generators[0])))
        return CayleyGraphDef.create(generators, central_state=central_state, generator_names=generator_names)
