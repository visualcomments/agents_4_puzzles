...
class PermutationGroups:
    ...
    def lsl_cycles(n: int, add_inverses: bool = True) -> CayleyGraphDef:
        ...
        add_inverses : bool, optional
            If True (default), include inverse permutations
            in the generator set to make the graph undirected.
        """
        assert n >= 3, "Need n >= 3"

        long_cycle = permutation_from_cycles(n, [list(range(n))])
        sub_long_cycle = permutation_from_cycles(n, [list(range(1, n)), [0]])

        generators = [long_cycle, sub_long_cycle]
        names = ["L", "S"]

        if add_inverses:
            long_cycle_inv = inverse_permutation(long_cycle)
            sub_long_cycle_inv = inverse_permutation(sub_long_cycle)

            generators += [long_cycle_inv, sub_long_cycle_inv]
            names += ["L_inv", "S_inv"]

        return CayleyGraphDef.create(
            generators, central_state=list(range(n)), generator_names=names, name=f"lsl_cycles-{n}"
        )
    ...
    @staticmethod
    def prefix_cycles(n: int) -> CayleyGraphDef:
        """
        Cayley graph generated by all prefix cycles in S_n:
        (0 1 ... j-1), j = 2..n.
        """
        assert n >= 2, "n must be >= 2"
...
def prepare_graph(name: str, n: int = 0, **kwargs) -> CayleyGraphDef:
    ...
    elif name == "prefix_cycles":
        return PermutationGroups.prefix_cycles(n)
    else:
