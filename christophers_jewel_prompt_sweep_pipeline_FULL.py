#!/usr/bin/env python3
"""
Standalone launcher for agents_4_puzzles Christophers Jewel prompt sweep.

This script was generated from the Colab notebook:
agents_4_puzzles_christophers_jewel_full_prompt_sweep_kaggle_feedback_FIXED_single_inline_kaggle_json.ipynb
...
5. writes the Christophers Jewel prompt-sweep runner into colab/;
...
def default_workspace_dir() -> str:
    ...
    env_value = os.environ.get("JEWEL_WORKDIR")
    if env_value:
        return str(Path(env_value).expanduser())
    return str(get_script_dir() / "christophers_jewel_prompt_sweep_workspace")


def default_archive_path() -> str:
    """Default zip path next to this script; used only for local_zip_path/upload_zip modes."""
    env_value = os.environ.get("JEWEL_ARCHIVE_PATH")
    if env_value:
        return str(Path(env_value).expanduser())
    return str(get_script_dir() / "agents_4_puzzles_christophers_jewel_prompt_sweep_pipeline_FULL.zip")
...
def ensure_writable_dir(path: Path, label: str) -> Path:
    ...
    try:
        ...
        fallback = (Path.cwd() / "christophers_jewel_prompt_sweep_workspace").resolve()
...
def write_runner(repo_dir: Path, *, overwrite: bool = True) -> Path:
    runner_path = repo_dir / "colab" / "christophers_jewel_prompt_sweep_kaggle_feedback.py"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Clone/unpack agents_4_puzzles, install dependencies, configure credentials, and run Christophers Jewel prompt sweep."
    )
    ...
    workspace.add_argument("--archive-path", default=default_archive_path(), help="Path to repository zip for local_zip_path/drive_zip/upload_zip modes. Defaults to a zip next to this script or JEWEL_ARCHIVE_PATH.")
    ...
    workspace.add_argument("--workdir", default=default_workspace_dir(), help="Writable workspace directory. Defaults to ./christophers_jewel_prompt_sweep_workspace next to this script or JEWEL_WORKDIR.")
    ...
    main.add_argument("--competition", default="cayleypy-christophers-jewel")
    main.add_argument("--kaggle-competition", default="cayleypy-christophers-jewel")
    ...
    main.add_argument("--run-name", default="christophers_jewel_full_prompt_sweep")
    ...
    main.add_argument("--output-root", default="runs/christophers_jewel_prompt_sweep")
    ...
    budget.add_argument("--baseline", default="competitions/cayleypy-christophers-jewel/baselines/jewel_baseline_solver.py")
    ...
    scoring.add_argument("--message-prefix", default="Christophers Jewel prompt sweep")
